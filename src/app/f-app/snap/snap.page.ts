...
export class SnapPage {
  ...
  constructor(
    ...
    this.offNickName = this.translate.instant("f_app." + this.fApp.name + ".username_off");
    this.subscribeInterval = interval(2000).subscribe(_ => this.updateList())
  }

  changeUser(event) {
    this.api.setSnapUsers({
    ...
    this.api.getCurrentFApp().subscribe((response) => {
      ...
      this.api.getSnapUsers().subscribe((res) => {
        ...
        this.clientList.push({
          id: "turnoff",
          username: this.translate.instant("f_app." + this.fApp.name + ".username_off"),
        });
      });
    });
  }
  ...
  public async showPopup() {
    const alert = await this.alertController.create({
      header: 'Alert',
      subHeader: this.translate.instant("f_app." + this.fApp.name + ".errors.title"),
      message: this.translate.instant("f_app." + this.fApp.name + ".errors.message"),
      buttons: [
        {
          text: 'OK', handler: () => {
